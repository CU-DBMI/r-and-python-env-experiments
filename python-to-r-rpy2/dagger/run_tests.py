    ...
    async with dagger.Connection(dagger.Config(log_output=sys.stderr)) as client:
        ...
        async def test_version(version: str):
            python = (
                ...
                .with_mounted_directory("/python-to-r-rpy2", src)
                # set current working directory for next commands
                .with_workdir("/python-to-r-rpy2")
                # install R base into container
                .with_exec(["apt", "update"])
                .with_exec(["apt", "install", "-y", "r-base"])
                # install test dependencies
                .with_exec(["pip", "install", "poetry==1.4"])
                .with_exec(["poetry", "install"])
                # run tests
                .with_exec(["poetry", "run", "python", "python/main.py"])
            )
